...
task('verify:tokens', 'Deploy oracles for dev enviroment')
  ...
  .setAction(async ({ verify, all, pool }, localDRE) => {
    ...
    for (const entry of Object.entries(getParamPerNetwork(ReserveAssets, network))) {
      ...
      // Proxy oToken
      console.log('\n- Verifying oToken proxy...\n');
      await verifyContract(
        eContractid.InitializableAdminUpgradeabilityProxy,
        await getProxy(oTokenAddress),
        [lendingPoolConfigurator.address]
      );
      ...
      const oToken = await getAddressById(`a${token}`);

      if (oToken) {
        console.log('\n- Verifying oToken...\n');
        await verifyContract(eContractid.OToken, await getOToken(oToken), [
          lendingPoolProxy.address,
          tokenAddress,
          treasuryAddress,
          `Meridian interest bearing ${token}`,
          `a${token}`,
          ZERO_ADDRESS,
        ]);
      } else {
        console.error(`Skipping oToken verify for ${token}. Missing address at JSON DB.`);
      }
      if (stableDebt) {
        ...
        await verifyContract(eContractid.StableDebtToken, await getStableDebtToken(stableDebt), [
          lendingPoolProxy.address,
          tokenAddress,
          `Meridian stable debt bearing ${token}`,
          `stableDebt${token}`,
          ZERO_ADDRESS,
        ]);
      } else {
      ...
      if (variableDebt) {
        ...
        await verifyContract(
          ...
          [
            lendingPoolProxy.address,
            tokenAddress,
            `Meridian variable debt bearing ${token}`,
            `variableDebt${token}`,
            ZERO_ADDRESS,
          ]
        );
