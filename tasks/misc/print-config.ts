...
task('print-config', 'Inits the DRE, to have access to all the plugins')
  .addParam('dataProvider', 'Address of MeridianProtocolDataProvider')
  ...
  .setAction(async ({ pool, dataProvider }, localBRE) => {
    ...
    const protocolDataProvider = await getMeridianProtocolDataProvider(dataProvider);
    ...
    const tokensFields = ['oToken', 'stableDebtToken', 'variableDebtToken'];
