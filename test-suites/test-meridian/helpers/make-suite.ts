...
export async function initializeMakeSuite() {
  ...
  if (process.env.FORK) {
    testEnv.registry = await getLendingPoolAddressesProviderRegistry(
      getParamPerNetwork(MeridianConfig.ProviderRegistry, process.env.FORK as eNetwork)
    );
  } else {
    testEnv.registry = await getLendingPoolAddressesProviderRegistry();
  ...
  const stlosAddress = reservesTokens.find((token) => token.symbol === 'STLOS')?.tokenAddress;
  const wethAddress = reservesTokens.find((token) => token.symbol === 'WETH')?.tokenAddress;

  if (!aDaiAddress || !aWEthAddress) {
    process.exit(1);
  }
  if (!daiAddress || !usdcAddress || !stlosAddress || !wethAddress) {
    process.exit(1);
  }
  ...
  testEnv.stlos = await getMintableERC20(stlosAddress);
