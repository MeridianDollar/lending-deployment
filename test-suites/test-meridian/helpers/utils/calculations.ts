...
interface Configuration {
  reservesParams: iMeridianPoolAssets<IReserveParams>;
}
...
export const calcExpectedUserDataAfterDeposit = (
  ...
  expectedUserData.scaledOTokenBalance = calcExpectedScaledOTokenBalance(
    userDataBeforeAction,
    reserveDataAfterAction.liquidityIndex,
    new BigNumber(amountDeposited),
    new BigNumber(0)
  );
  expectedUserData.currentOTokenBalance = calcExpectedOTokenBalance(
    reserveDataBeforeAction,
    userDataBeforeAction,
    txTimestamp
  ).plus(amountDeposited);

  if (userDataBeforeAction.currentOTokenBalance.eq(0)) {
...
export const calcExpectedUserDataAfterWithdraw = (
  ...
  const oTokenBalance = calcExpectedOTokenBalance(
    reserveDataBeforeAction,
    userDataBeforeAction,
    txTimestamp
  );
  ...
  if (amountWithdrawn == MAX_UINT_AMOUNT) {
    amountWithdrawn = oTokenBalance.toFixed(0);
  }
  ...
  expectedUserData.currentOTokenBalance = oTokenBalance.minus(amountWithdrawn);
  ...
  if (userDataBeforeAction.currentOTokenBalance.eq(0)) {
    ...
    if (expectedUserData.currentOTokenBalance.eq(0)) {
...
export const calcExpectedReserveDataAfterWithdraw = (
  ...
  if (amountWithdrawn == MAX_UINT_AMOUNT) {
    amountWithdrawn = calcExpectedOTokenBalance(
      reserveDataBeforeAction,
      userDataBeforeAction,
      txTimestamp
    ).toFixed();
...
export const calcExpectedUserDataAfterBorrow = (
  ...
  expectedUserData.scaledOTokenBalance = userDataBeforeAction.scaledOTokenBalance;
...
const calcExpectedScaledOTokenBalance = (
  userDataBeforeAction: UserReserveData,
  index: BigNumber,
  amountAdded: BigNumber,
  amountTaken: BigNumber
) => {
  return userDataBeforeAction.scaledOTokenBalance
    .plus(amountAdded.rayDiv(index))
    .minus(amountTaken.rayDiv(index));
};

export const calcExpectedOTokenBalance = (
  ...
  const { scaledOTokenBalance: scaledBalanceBeforeAction } = userData;
