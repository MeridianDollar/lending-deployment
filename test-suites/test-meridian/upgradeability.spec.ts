...
makeSuite('Upgradeability', (testEnv: TestEnv) => {
  ...
  before('deploying instances', async () => {
    ...
    const oTokenInstance = await deployMockOToken([
      ...
      ZERO_ADDRESS,
      ZERO_ADDRESS,
      'Meridian Interest bearing DAI updated',
      'oDAI',
      '0x10',
    ]);
    ...
    const stableDebtTokenInstance = await deployMockStableDebtToken([
      ...
      dai.address,
      ZERO_ADDRESS,
      'Meridian stable debt bearing DAI updated',
      'stableDebtDAI',
      '0x10',
    ]);
    ...
    const variableDebtTokenInstance = await deployMockVariableDebtToken([
      ...
      dai.address,
      ZERO_ADDRESS,
      'Meridian variable debt bearing DAI updated',
      'variableDebtDAI',
      '0x10',
    ]);
  ...
  it('Upgrades the DAI Otoken implementation ', async () => {
    ...
    expect(tokenName).to.be.eq('Meridian Interest bearing DAI updated', 'Invalid token name');
  });
  ...
  it('Upgrades the DAI stable debt token implementation ', async () => {
    ...
    expect(tokenName).to.be.eq('Meridian stable debt bearing DAI updated', 'Invalid token name');
  });
  ...
  it('Upgrades the DAI variable debt token implementation ', async () => {
    ...
    expect(tokenName).to.be.eq('Meridian variable debt bearing DAI updated', 'Invalid token name');
  });
});
