...
const MOCK_USD_PRICE_IN_WEI = MeridianConfig.ProtocolGlobalParams.MockUsdPriceInWei;
const ALL_ASSETS_INITIAL_PRICES = MeridianConfig.Mocks.AllAssetsInitialPrices;
const USD_ADDRESS = MeridianConfig.ProtocolGlobalParams.UsdAddress;
const LENDING_RATE_ORACLE_RATES_COMMON = MeridianConfig.LendingRateOracleRatesCommon;
...
const buildTestEnv = async (deployer: Signer, secondaryWallet: Signer) => {
  ...
  const config = loadPoolConfig(ConfigNames.Meridian);
  ...
  const addressesProvider = await deployLendingPoolAddressesProvider(MeridianConfig.MarketId);
  ...
  // Reserve params from STLOS pool + mocked tokens
  ...
  await deployOTokenImplementations(ConfigNames.Meridian, reservesParams, false);
...
before(async () => {
  ...
  if (FORK) {
    await rawBRE.run('meridian:mainnet', { skipRegistry: true });
  } else {
