...
contract MeridianProtocolDataProvider {
  ...
  function getAllOTokens() external view returns (TokenData[] memory) {
    ILendingPool pool = ILendingPool(ADDRESSES_PROVIDER.getLendingPool());
    address[] memory reserves = pool.getReservesList();
    TokenData[] memory oTokens = new TokenData[](reserves.length);
    for (uint256 i = 0; i < reserves.length; i++) {
      DataTypes.ReserveData memory reserveData = pool.getReserveData(reserves[i]);
      oTokens[i] = TokenData({
        symbol: IERC20Detailed(reserveData.oTokenAddress).symbol(),
        tokenAddress: reserveData.oTokenAddress
      });
    }
    return oTokens;
  }
    ...
    )
  {
    ...
    return (
      IERC20Detailed(asset).balanceOf(reserve.oTokenAddress),
    ...
    )
  {
    ...
    currentOTokenBalance = IERC20Detailed(reserve.oTokenAddress).balanceOf(user);
