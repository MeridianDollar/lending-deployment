...
contract UniswapLiquiditySwapAdapter is BaseUniswapAdapter {
   ...
   * The user should give this contract allowance to pull the OTokens in order to withdraw the underlying asset and
  ...
  struct SwapAndDepositLocalVars {
    uint256 i;
    uint256 oTokenInitiatorBalance;
  ...
  function swapAndDeposit(
    ...
    for (vars.i = 0; vars.i < assetToSwapFromList.length; vars.i++) {
      vars.oToken = _getReserveData(assetToSwapFromList[vars.i]).oTokenAddress;

      vars.oTokenInitiatorBalance = IERC20(vars.oToken).balanceOf(msg.sender);
      vars.amountToSwap = amountToSwapList[vars.i] > vars.oTokenInitiatorBalance
        ? vars.oTokenInitiatorBalance
  ...
  function _swapLiquidity(
    ...
    vars.oToken = _getReserveData(assetFrom).oTokenAddress;

    vars.oTokenInitiatorBalance = IERC20(vars.oToken).balanceOf(initiator);
    vars.amountToSwap = swapAllBalance && vars.oTokenInitiatorBalance.sub(premium) <= amount
      ? vars.oTokenInitiatorBalance.sub(premium)
      : amount;
    ...
    _pullOToken(assetFrom, vars.oToken, initiator, vars.amountToPull, permitSignature);
