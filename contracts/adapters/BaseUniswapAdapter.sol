...
abstract contract BaseUniswapAdapter is FlashLoanReceiverBase, IBaseUniswapAdapter, Ownable {
  ...
  /**
   * @dev Get the oToken associated to the asset
   * @return address of the oToken
   */
  ...
  /**
   * @dev Pull the OTokens from the user
   * @param reserve address of the asset
   * @param reserveOToken address of the oToken of the reserve
   * @param user address
   * @param amount of tokens to be transferred to the contract
   * @param permitSignature struct containing the permit signature
   */
  function _pullOToken(
    address reserve,
    address reserveOToken,
    address user,
    uint256 amount,
    PermitSignature memory permitSignature
  ) internal {
    if (_usePermit(permitSignature)) {
      IERC20WithPermit(reserveOToken).permit(
        user,
        address(this),
    ...
    IERC20(reserveOToken).safeTransferFrom(user, address(this), amount);
