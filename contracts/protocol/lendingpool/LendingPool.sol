...
contract LendingPool is VersionedInitializable, ILendingPool, LendingPoolStorage {
  ...
  function deposit(
    ...
    address oToken = reserve.oTokenAddress;

    reserve.updateState();
    reserve.updateInterestRates(asset, oToken, amount, 0);

    IERC20(asset).safeTransferFrom(msg.sender, oToken, amount);

    bool isFirstDeposit = IOToken(oToken).mint(onBehalfOf, amount, reserve.liquidityIndex);
  ...
  function withdraw(
    ...
    reserve.updateInterestRates(asset, oToken, 0, amountToWithdraw);
    ...
    IOToken(oToken).burn(msg.sender, to, amountToWithdraw, reserve.liquidityIndex);
  ...
  function repay(
    ...
    address oToken = reserve.oTokenAddress;
    reserve.updateInterestRates(asset, oToken, paybackAmount, 0);

    if (stableDebt.add(variableDebt).sub(paybackAmount) == 0) {
      _usersConfig[onBehalfOf].setBorrowing(reserve.id, false);
    }

    IERC20(asset).safeTransferFrom(msg.sender, oToken, paybackAmount);

    IOToken(oToken).handleRepayment(msg.sender, paybackAmount);
  ...
  function swapBorrowRateMode(address asset, uint256 rateMode) external override whenNotPaused {
    ...
    reserve.updateInterestRates(asset, reserve.oTokenAddress, 0, 0);
  ...
  function flashLoan(
    ...
    address[] memory oTokenAddresses = new address[](assets.length);
    uint256[] memory premiums = new uint256[](assets.length);

    vars.receiver = IFlashLoanReceiver(receiverAddress);

    for (vars.i = 0; vars.i < assets.length; vars.i++) {
      oTokenAddresses[vars.i] = _reserves[assets[vars.i]].oTokenAddress;

      premiums[vars.i] = amounts[vars.i].mul(_flashLoanPremiumTotal).div(10000);

      IOToken(oTokenAddresses[vars.i]).transferUnderlyingTo(receiverAddress, amounts[vars.i]);
    }
    ...
    for (vars.i = 0; vars.i < assets.length; vars.i++) {
      ...
      vars.currentOTokenAddress = oTokenAddresses[vars.i];
      vars.currentAmountPlusPremium = vars.currentAmount.add(vars.currentPremium);

      if (DataTypes.InterestRateMode(modes[vars.i]) == DataTypes.InterestRateMode.NONE) {
        _reserves[vars.currentAsset].updateState();
        _reserves[vars.currentAsset].cumulateToLiquidityIndex(
          IERC20(vars.currentOTokenAddress).totalSupply(),
          vars.currentPremium
        );
        _reserves[vars.currentAsset].updateInterestRates(
          vars.currentAsset,
          vars.currentOTokenAddress,
  ...
  /**
   * @dev Validates and finalizes an oToken transfer
   * - Only callable by the overlying oToken of the `asset`
   * @param asset The address of the underlying asset of the oToken
   * @param from The user from which the oTokens are transferred
   * @param to The user receiving the oTokens
   * @param amount The amount being transferred/withdrawn
   * @param balanceFromBefore The oToken balance of the `from` user before the transfer
   * @param balanceToBefore The oToken balance of the `to` user before the transfer
   */
  function finalizeTransfer(
    ...
    require(msg.sender == _reserves[asset].oTokenAddress, Errors.LP_CALLER_MUST_BE_AN_OTOKEN);
  ...
  /**
   * @dev Initializes a reserve, activating it, assigning an oToken and debt tokens and an
   * interest rate strategy
   * - Only callable by the LendingPoolConfigurator contract
   * @param asset The address of the underlying asset of the reserve
   * @param oTokenAddress The address of the oToken that will be assigned to the reserve
   * @param stableDebtAddress The address of the StableDebtToken that will be assigned to the reserve
   * @param oTokenAddress The address of the VariableDebtToken that will be assigned to the reserve
  ...
  function _executeBorrow(ExecuteBorrowParams memory vars) internal {
    ...
    if (vars.releaseUnderlying) {
      IOToken(vars.oTokenAddress).transferUnderlyingTo(vars.user, vars.amount);
    }
